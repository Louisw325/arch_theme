...
# Create lsd directory if it doesn't exist
mkdir -p "$LSD_DIR"

# Source the theme
source "$THEME_FILE"

# Function to convert hex to nearest 256-color code
hex_to_256() {
    local hex="${1#\#}"
    local r=$((16#${hex:0:2}))
    local g=$((16#${hex:2:2}))
    ...
    # Simple conversion - for more accuracy, use a lookup table
    # This maps RGB to xterm-256 color palette
    if [ $r -eq $g ] && [ $g -eq $b ]; then
        # Grayscale
        if [ $r -lt 8 ]; then
            echo 16
        ...
        # Color - use 6x6x6 color cube
        local ir=$(( r < 48 ? 0 : r < 115 ? 1 : (r - 55) / 40 ))
        local ig=$(( g < 48 ? 0 : g < 115 ? 1 : (g - 55) / 40 ))
        local ib=$(( b < 48 ? 0 : b < 115 ? 1 : (b - 55) / 40 ))
        echo $(( 16 + 36 * ir + 6 * ig + ib ))
    fi
}

# Convert theme colors to 256-color codes
PRIMARY_256=$(hex_to_256 "$PRIMARY")
ACCENT_256=$(hex_to_256 "$ACCENT")
SUCCESS_256=$(hex_to_256 "$SUCCESS")
WARNING_256=$(hex_to_256 "$WARNING")
ERROR_256=$(hex_to_256 "$ERROR")
INFO_256=$(hex_to_256 "$INFO")
FG_256=$(hex_to_256 "$FG")
FG_DIM_256=$(hex_to_256 "$FG_DIM")
URGENT_256=$(hex_to_256 "$URGENT")

# Generate lsd colors configuration
OUTPUT_FILE="$LSD_DIR/colors.yaml"

cat > "$OUTPUT_FILE" << EOF
# lsd colors - Generated from $THEME_NAME theme
# Using 256-color palette codes for compatibility with lsd 1.2.0
user: $PRIMARY_256
group: $ACCENT_256
permission:
  read: $SUCCESS_256
...
git-status:
  ...
  unmodified: $FG
  ignored: $FG_DIM
  new-in-index: $SUCCESS
  new-in-workdir: $SUCCESS
  typechange: $WARNING
  deleted: $ERROR
  renamed: $ACCENT
  modified: $WARNING
...
echo "✓ Generated lsd color configuration: $OUTPUT_FILE"
